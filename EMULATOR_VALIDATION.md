...
`DMGWYE-0.781` ROM** using the emulators in this folder — no hardware needed. The
...
## Status (done here)
- GBE+ configured for WorkBoy: `gbe_1_10/CURRENT/gbe.ini` → `[#sio_device:22]`
  (original saved as `gbe.ini.bak`).
...
1. From `gbe_1_10/CURRENT/`, run the Qt build for menus:
   ```
   gbe_plus_qt.exe
   ```
...
5. This is the ground truth our **firmware** reproduces: GBE+'s `workboy_process`
   (in `gbe-plus-master/src/dmg/sio.cpp`) is the exact state machine our
   `firmware/workboy_keyboard.c` mirrors, so a keypress that works here works on
   the real hardware too.

## Cross-check in SameBoy
`sameboy_winsdl_v1.0.3/sameboy.exe` also implements a WorkBoy device (`Core/workboy.c`).
